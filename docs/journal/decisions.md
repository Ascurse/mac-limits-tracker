...
## 2026-07-28 — Hybrid scene + WindowPresentationController: один источник истины для Dock-иконки (bd mac-limits-tracker-3ip.4)

**Контекст:** приложение жило как menu-bar-only (`.accessory`/`LSUIElement=true`, AppDelegate ставил `NSApp.setActivationPolicy(.accessory)` в `applicationDidFinishLaunching`) — Dock-иконки не было, и добавить singleton desktop window без него было нельзя: единственный способ попасть в `.regular` — убрать `LSUIElement` и переключать `NSApplication.ActivationPolicy` вручную. Параллельно сводный `LimitsViewModel` (3ip.1) уже идемпотентен и готов делиться состоянием между поверхностями, а `ProviderOverview` (3ip.2) уже умеет рендериться под `surface: .desktop` без новых правил форматирования.
**Решение:** `WindowPresentationController` в Core (без AppKit-зависимостей) — класс с `private(set) var isMainWindowPresented: Bool` и инжектируемой closure `apply: (ActivationPolicy) -> Void`. App-таргет маппит `ActivationPolicy` в `NSApplication.ActivationPolicy` и зовёт `ensureAccessoryOnLaunch()` из `AppDelegate.applicationDidFinishLaunching` (NSApp в `MacLimitsTrackerApp.init` ещё nil — см. gotcha 2026-07-28). `Window("Limits Tracker", id: "main") { DesktopWindowView(...) }` — singleton scene SwiftUI 4+: повторный `openWindow(id: "main")` фокусирует существующее окно, закрытие не завершает app, `.onAppear/.onDisappear` дёргают `setMainWindowPresented`. App-команда «Open Limits Tracker» (Cmd-0) в `.commands { ... }` + кнопка в `StatusBarView` (top bar) используют `@Environment(\.openWindow)`. `LSUIElement` выкинут из `make-app.sh` Info.plist; политика теперь полностью runtime.
**Почему:** сцены Singleton через `Window` дешевле и нативнее, чем свой `NSPanel` или `NSWindowController` — закрытие и фокус уже корректны, singleton-семантика приходит бесплатно. Controller в Core, а не в App, чтобы юнит-тесты могли гонять его без поднятия NSApp; маппинг на AppKit-тип остаётся в app-таргете в одном месте. `setMainWindowPresented` идемпотентен, чтобы повторный `.onAppear` (типичный артефакт переоткрытия popup) не дёргал `NSApp.setActivationPolicy` без надобности. Cmd-0 выбран как естественный «Cmd-N для окна» паттерн, не конфликтует с системными шорткатами.
**Последствия:** LSUIElement снят с `.app` — стартовый frame процесса кратко показывает Dock-иконку до того, как AppDelegate применит `.accessory` (визуально незаметно, Dock-иконка появляется/исчезает в пределах одного кадра). Desktop window сейчас переиспользует ту же `LimitsViewModel` и тот же `ProviderOverview` — отдельных настроек у окна нет, footer/settings — задача 3ip.3. Любые будущие окна (например, Settings scene) обязаны проходить через тот же контроллер, иначе две независимые policy-ноги передерутся. Пин-тесты: `WindowPresentationControllerTests` (8 шт.) — default state, idempotency true/true, true→false, false/false от default, ensureAccessoryOnLaunch, lifecycle open→close→open.
