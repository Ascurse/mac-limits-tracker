...
**Почему:** единый контракт сохраняет порядок провайдеров и одинаковые правила расчёта во всех четырёх темах, не дублируя фильтрацию в SwiftUI.
**Последствия:** история продолжает собираться для forecast; ручная AX-проверка bounded popup остаётся отдельным QA gate.

---

## 2026-08-11 — Тренд и дневной ориентир управляются глобальными настройками [пересмотрено: 2026-08-12]

**Контекст:** после появления истории приложение показывало тренд одинаково во всех темах и поверхностях, а недельный лимит не подсказывал безопасный темп расходования.
**Решение:** `showDailyBudget` остаётся глобальной persist-настройкой с дефолтом `true`, применяемой до рендера общего `PopupContent`; `showUsageTrends` больше не является пользовательской настройкой. Дневной ориентир рассчитывается только для свежего weekly-окна с актуальным `resetsAt` и показывается на menu-bar и desktop поверхностях.
**Почему:** единый presentation-контракт сохраняет одинаковое поведение popup, desktop window и всех тем; переключатели не запускают refresh и не меняют историю.
**Последствия:** daily budget не является отдельным лимитом и не показывается для stale-данных, окон без даты сброса или 5h-окон; pace comparison остаётся desktop-only.

## 2026-08-11 — Оценка стоимости читается только из локальных логов

**Контекст:** пользователю нужна ориентировочная стоимость использования Claude Code и Codex без сетевого запроса к биллингу.
**Решение:** `CostEstimator` читает локальные логи и добавляет синтетическую секцию `Cost estimate` во все общие popup-поверхности. Если часть записей не удалось оценить, показывается нижняя граница с явной пометкой; отсутствие пригодных логов — отдельное состояние.
**Почему:** локальный read-only источник не требует новых credentials или provider API и не смешивает оценку стоимости с live-лимитами.
**Последствия:** сумма не является счётом провайдера; pricing table и формат USD остаются частью Core-контракта.
