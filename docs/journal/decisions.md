...
## 2026-08-11 — Тренд и дневной ориентир управляются глобальными настройками
...
**Решение:** `showUsageTrends` и `showDailyBudget` — глобальные persist-настройки с дефолтом `true`, применяемые до рендера общего `PopupContent`. Тренд строится за 7 дней; при выключении заменяется компактной строкой, а дневной ориентир рассчитывается только для weekly-окна с актуальным `resetsAt`.
...
**Последствия:** README и UI должны называть график 7-дневным; daily budget не является отдельным лимитом и не показывается для stale-данных или окон без даты сброса.
