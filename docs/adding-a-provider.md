...
(по образцу Claude/Codex; Kimi — самый свежий пример, прошедший путь от
«тонкого» провайдера без usage до полного live-источника — bd
mac-limits-tracker-6gk.3 → 6gk.8) и не хочет читать весь стек кода. Изучать нужно
...
Если у провайдера **нет источника usage** (нет ни файла, ни CLI, ни API) —
это нормально: оставь `windows` в снапшоте `nil`, а `usageError` объяснит
пользователю, что данных не будет (не `"Loading…"` — это подразумевало бы,
что они появятся). Если источник есть, но временно недоступен (протух токен,
сеть, 401) — тот же приём: `windows: nil` + понятный `usageError`, при этом
`loggedIn` можно оставить `true`. Живой пример HTTP-источника с graceful-
деградацией — `KimiLimitsProvider.fetchUsage()` (`GET /coding/v1/usages`,
обработка 401/истёкшего токена, bd mac-limits-tracker-6gk.8).
